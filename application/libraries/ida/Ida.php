...
class Ida{
    ...
    */ 
    public function getActInfo(){
        ...
        if (!self::$_db){
            self::$_ci->load->library('ida/database');
            self::$_db = self::$_ci->database->conn();
        }

        if (!self::$_mc){
            self::$_ci->load->library('ida/cache');
    ...
    */ 
    public function getSchoolInfo(){
        ...
        foreach ($school['values'] as $value){
            $school_info['type_sum'][$value] = self::$_db->cloud_teaching->user->find(array('user_school' => $value))->count();
        }
